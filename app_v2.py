...
if page == "Dashboard Home":
    st.markdown("<h2 style='font-weight: 700; font-size: 2.25rem; font-family: monospace; margin-bottom: 8px; color: #ffffff;'>Platform Overview & Telemetry</h2>", unsafe_allow_html=True)
    st.markdown("<p style='color: #94a3b8; font-size: 0.9rem; margin-bottom: 32px;'>Digital media forensics monitor combining classic, high-performance mathematical machine learning models backed by SQLite transaction logs.</p>", unsafe_allow_html=True)

    # Load statistics from prediction_history table
    df_history = get_prediction_history()
    total_scans = len(df_history)
    fake_news_total = len(df_history[df_history["type"] == "news"]) if total_scans > 0 else 0
    deepfake_total = len(df_history[df_history["type"] == "image"]) if total_scans > 0 else 0
    
    abnormal_scans = len(df_history[df_history["result_classifier"] == "FAKE"]) if total_scans > 0 else 0
    ...
    with col_chart1:
        ...
        if total_scans == 0:
            ...
            # Transform history into timeline data
            df_history["date_str"] = pd.to_datetime(df_history["timestamp"]).dt.strftime('%b %d')
            timeline = df_history.groupby(["date_str", "result_classifier"]).size().unstack(fill_value=0)
    ...
    with col_chart2:
        ...
        if total_scans == 0:
            ...
            distribution = df_history["result_classifier"].value_counts()
...
# ==========================================
    ...
    st.markdown("""
        ### Framework Foundations & Core Purpose
        This application acts as a high-fidelity digital media forensic workspace, targeted at vetting visual models and articles.
        
        #### Included Pipelines:
        
        1. **Fake News Classifier (Logistic Regression)**
           * **Concept**: Converts raw text columns into structured token arrays. It uses pre-computed logit weights corresponding to vocabulary mappings to compute the probability of disinformation.
           * **Limitations**: Highly sensitive to explicit keyword profiles. May miscalculate complex, subtle satire, or nuanced sarcasm.
 
        2. **Deepfake Visual Spotter (Random Forest Classifier)**
           * **Concept**: Integrates an ensemble pipeline compiling votes across 5 separate uncorrelated Decision Trees. Evaluating pixel matrices directly via OpenCV, the ensemble registers a prediction.
           * **Limitations**: Operates upon flattened matrices. Extremely small or heavily compressed images might blur sensor micro-noise, dropping verification accuracy.
 
        3. **Persistent Transaction History (SQLite Database Integration)**
           * **Buffer**: Backed by a file-persistent `news.db` SQLite connection under the `prediction_history` table, each calculation commits a chronological log structure allowing forensic teams to audit classifications across active operations.
    """)
